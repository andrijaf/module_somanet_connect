...
[[combinable]]
void somanet_connect_server(chanend c_host_data, client interface somanet_connect_plugin_interface scpi[n], unsigned n) {
        ...
        select {
            case xscope_data_from_host(c_host_data, (unsigned char *)buffer, bytes_read): {
                ...
                unsafe {
                    ...
                    for (int i = 0; i < n; i++) {
                        if (scpi[i].get_type() == type) {
                            unsigned char command[256];
                            memcpy(command, ++ptr, (bytes_read - 1)*sizeof(unsigned char));
                            scpi[i].get_command(command, bytes_read - 1);
                        }
                    }
                }
                break;
